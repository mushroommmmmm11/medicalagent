...
Answer contract: {response_contract}"""
    ...
    def _is_recovery_time_question(self, query: str) -> bool:
        normalized = query or ""
        return any(
    ...
    def _build_recovery_time_answer(self) -> str:
        return (
            "????????????????????/?????CRP 18.11 mg/L ???"
            "??? 11.42?10^9/L ????????????????????????"
            "???????????????????????????????????"
    ...
    def process_query(self, query: str, user_context: Optional[str] = None, lab_results: Optional[Dict] = None):
        try:
            if self._is_recovery_time_question(query):
                return self._build_recovery_time_answer(), []
